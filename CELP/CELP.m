...
% Let us load file 'Scooby1.wav', listen to it, and plot its samples. This
...
%%
% As such, however, the Scooby1.waveform is not "readable", even by an
...
speech=audioread('Scooby1.wav');
speech=speech(:,1);
input_frame=speech(384:384+239);
figure(3)
plot(input_frame);
xlabel('Time (samples)'); ylabel('Amplitude');

%%
% As expected this sound is approximately periodic (period=72 samples, i.e.
% 80 ms; fundamental frequency = 147 Hz). Notice, though, that this is only
...
pause
%%
% The fundamental frequency appears again at around 110 Hz. One can also
...
% of pulses separated by 72 zeros (so as to impose a period of 65 samples).
% Notice we multiply the excitation by some gain so that its variance
% matches that of the residual signal. 

excitation = [1;zeros(72,1);1;zeros(72,1);1;zeros(72,1);1;zeros(44,1)];
gain=sigma/sqrt(1/73);
figure(11)
...
speech=audioread('Scooby1.wav');
speech=speech(:,1);
input_frame=speech(9800:9800+239);
figure(15)
plot(input_frame);
xlabel('Time (samples)'); ylabel('Amplitude');
...
for i=1:(length(speech)-160)/80; % number of frames
    ...
    synt_speech_V=[synt_speech_V;synt_frame]; %#ok<AGROW> 
...
figure(20)
plot(synt_speech_V(384:384+239));
xlabel('Time (samples)'); ylabel('Amplitude');
...
N0=72;   % Constant synthesis pitch period (in samples)
